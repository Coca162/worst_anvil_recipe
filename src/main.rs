...
fn main() {
    for protections in PROTECTIONS {
        dbg!(INCLUSIVE_ENCHANTMENTS.into_iter().chain(iter::once(protections)).flat_map(|e| {
            iter::repeat_n(Item::from_enchantment(e), 2_u32.pow(e.max_level) as usize)
        }).count());    

        let max = INCLUSIVE_ENCHANTMENTS.into_iter().chain(iter::once(protections)).flat_map(|e| {
            iter::repeat_n(Item::from_enchantment(e), e.max_level as usize)
        }).permutations(10).par_bridge().filter_map(|permutations| {
            ...
            while permutations.len() != 1 {
                ...
                while let (Some(t), Some(e)) = (permutations.next(), permutations.next()) {
                    match t.combine(e) {
                        Ok(e) => vec.push(e),
                        Err(AnvilError::RedundantSequence | AnvilError::TooExpensive) => return None,
                        // Err(err) => panic!("{err:?}")
                    }
                }
